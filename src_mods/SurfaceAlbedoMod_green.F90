...
contains
  ...
  !-----------------------------------------------------------------------
   ...
   subroutine TwoStream (bounds, &
     ...
     real(r8) :: extkn                                             ! nitrogen allocation coefficient
     real(r8) :: fsun_green(bounds%begp:bounds%endp)               ! green leaf canopy fraction for nitrogen scaling
     !-----------------------------------------------------------------------
...
!Modified by Ming 11/05/2016 and Ethan 04/15/2020
                extkn = 0.30_r8
                extkb = twostext(p)
                if (elai(p) > 0._r8) then
                   fsun_green(p) = (1._r8 -exp(-extkb*elai(p)))*(1-exp(-extkb*esai(p)))/(extkb*extkb*esai(p)*elai(p))
                end if
!End of change
...
!End of change
! Changed by Ethan 04/15/2020
                if (elai(p)  >  0._r8) then
                  vcmaxcintsun(p) = vcmaxcintsun(p) / (fsun_green(p)*elai(p))
                  vcmaxcintsha(p) = vcmaxcintsha(p) / ((1._r8 - fsun_green(p))*elai(p))
                else
                  vcmaxcintsun(p) = 0._r8
                  vcmaxcintsha(p) = 0._r8
                end if
